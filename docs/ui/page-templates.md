...
- metadados de compartilhamento (`og:image`/`twitter:image`) devem priorizar a imagem do hero configurada no tenant; quando a view já possui fallback tenant-scoped de produto, pode usar essa imagem sem consultar catálogo fora do tenant; na ausência de mídia real, usar `Tenant.logo_url` antes de publicar placeholder
