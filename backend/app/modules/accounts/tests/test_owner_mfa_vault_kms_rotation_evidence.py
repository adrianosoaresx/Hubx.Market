...
class OwnerMfaVaultKmsRotationEvidenceTests(TestCase):
    secret = "GEZDGNBVGY3TQOJQ"

    def setUp(self):
        self.canary = Tenant.objects.create(name="MFA Vault Rotation Evidence Canary", slug="mfa-vault-rotation-evidence-canary", subdomain="mfa-vault-rotation-evidence-canary")
        self.current = Tenant.objects.create(name="MFA Vault Rotation Evidence Current", slug="mfa-vault-rotation-evidence-current", subdomain="mfa-vault-rotation-evidence-current")
        self.next_tenant = Tenant.objects.create(name="MFA Vault Rotation Evidence Next", slug="mfa-vault-rotation-evidence-next", subdomain="mfa-vault-rotation-evidence-next")
        self.owner = OwnerUser.objects.create(tenant=self.canary, email="owner.mfa.vault.rotation.evidence@hubx.market", role="owner")
    ...
    def test_evidence_ready_when_runbook_and_rotation_confirmations_are_ready(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation-evidence/ready")
        ...
        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            evidence = owner_mfa_vault_kms_rotation_evidence_queries.get_evidence(
                **self._ready_kwargs(probe_reference="owners/hashicorp-rotation-evidence/ready")
            )

        self.assertTrue(evidence["ready"])
        self.assertEqual(evidence["status"], "ready")
        self.assertEqual(evidence["blockers"], ())
        self.assertIn("Owner MFA Vault/KMS Post-Rotation Monitoring Review", evidence["next_tracks"])
    ...
    def test_evidence_blocks_when_post_rotation_probe_is_missing(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation-evidence/blocked")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        kwargs = self._ready_kwargs(probe_reference="owners/hashicorp-rotation-evidence/blocked")
        kwargs["post_rotation_probe_passed"] = False

        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            evidence = owner_mfa_vault_kms_rotation_evidence_queries.get_evidence(**kwargs)

        self.assertFalse(evidence["ready"])
        self.assertIn("confirmation-missing:post_rotation_probe_passed", evidence["blockers"])
    ...
    def test_command_outputs_evidence_without_secret_or_path(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation-evidence/command")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        output = StringIO()

        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            call_command(
                "owner_mfa_vault_kms_rotation_evidence",
                "--canary-tenant-id",
                str(self.canary.id),
                ...
                "--probe-reference",
                "owners/hashicorp-rotation-evidence/command",
                "--canary-owner-email",
                self.owner.email,
                *self._ready_flags(),
                stdout=output,
        ...
        self.assertIn("evidence=rotation_executed=True", output.getvalue())
        self.assertIn("decision key=classification", output.getvalue())
        self.assertNotIn(self.secret, output.getvalue())
        self.assertNotIn("owners/hashicorp-rotation-evidence/command", output.getvalue())
