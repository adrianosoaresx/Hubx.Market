...
class OwnerMfaVaultKmsRotationRunbookTests(TestCase):
    secret = "GEZDGNBVGY3TQOJQ"

    def setUp(self):
        self.canary = Tenant.objects.create(name="MFA Vault Rotation Canary", slug="mfa-vault-rotation-canary", subdomain="mfa-vault-rotation-canary")
        self.current = Tenant.objects.create(name="MFA Vault Rotation Current", slug="mfa-vault-rotation-current", subdomain="mfa-vault-rotation-current")
        self.next_tenant = Tenant.objects.create(name="MFA Vault Rotation Next", slug="mfa-vault-rotation-next", subdomain="mfa-vault-rotation-next")
        self.owner = OwnerUser.objects.create(tenant=self.canary, email="owner.mfa.vault.rotation@hubx.market", role="owner")
    ...
    def test_runbook_ready_when_closure_and_rotation_signals_are_ready(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation/ready")
        ...
        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            review = owner_mfa_vault_kms_rotation_runbook_queries.get_review(
                **self._ready_kwargs(probe_reference="owners/hashicorp-rotation/ready")
            )

        self.assertTrue(review["ready"])
        self.assertEqual(review["status"], "ready")
        self.assertEqual(review["blockers"], ())
        self.assertIn("Owner MFA Vault/KMS Rotation Evidence Execution", review["next_tracks"])
    ...
    def test_runbook_blocks_when_vault_access_is_missing(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation/blocked")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        kwargs = self._ready_kwargs(probe_reference="owners/hashicorp-rotation/blocked")
        kwargs["vault_access_validated"] = False

        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            review = owner_mfa_vault_kms_rotation_runbook_queries.get_review(**kwargs)

        self.assertFalse(review["ready"])
        self.assertIn("runbook:vault_access_validated:missing", review["blockers"])
    ...
    def test_command_outputs_runbook_without_secret_or_path(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation/command")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        output = StringIO()

        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            call_command(
                "owner_mfa_vault_kms_rotation_runbook",
                "--canary-tenant-id",
                str(self.canary.id),
                ...
                "--probe-reference",
                "owners/hashicorp-rotation/command",
                "--canary-owner-email",
                self.owner.email,
                *self._ready_flags(),
                stdout=output,
        ...
        self.assertIn("runbook_signal key=rotation_scope_documented value=True", output.getvalue())
        self.assertIn("decision key=classification", output.getvalue())
        self.assertNotIn(self.secret, output.getvalue())
        self.assertNotIn("owners/hashicorp-rotation/command", output.getvalue())
        self.assertNotIn("vault-token", output.getvalue())

    def test_command_can_fail_when_runbook_is_blocked(self):
