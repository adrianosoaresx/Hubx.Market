...
class OwnerMfaVaultKmsRotationClosureTests(TestCase):
    secret = "GEZDGNBVGY3TQOJQ"

    def setUp(self):
        self.canary = Tenant.objects.create(
            name="MFA Vault Rotation Closure Canary",
            slug="mfa-vault-rotation-closure-canary",
            subdomain="mfa-vault-rotation-closure-canary",
        )
        self.current = Tenant.objects.create(
            name="MFA Vault Rotation Closure Current",
            slug="mfa-vault-rotation-closure-current",
            subdomain="mfa-vault-rotation-closure-current",
        )
        self.next_tenant = Tenant.objects.create(
            name="MFA Vault Rotation Closure Next",
            slug="mfa-vault-rotation-closure-next",
            subdomain="mfa-vault-rotation-closure-next",
        )
        self.owner = OwnerUser.objects.create(
            tenant=self.canary,
            email="owner.mfa.vault.rotation.closure@hubx.market",
            role="owner",
        )
    ...
    def test_closure_ready_when_monitoring_and_closure_signals_are_clean(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation-closure/ready")
        ...
        with patch(
            ...
            closure = owner_mfa_vault_kms_rotation_closure_queries.get_closure(
                **self._ready_kwargs(probe_reference="owners/hashicorp-rotation-closure/ready"),
                **self._closure_kwargs(),
            )

        self.assertTrue(closure["ready"])
        self.assertEqual(closure["result"], "owner-mfa-vault-kms-rotation-closure-ready")
    ...
    def test_closure_blocks_when_evidence_archive_is_missing(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation-closure/blocked")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        closure_kwargs = self._closure_kwargs()
        closure_kwargs["rotation_evidence_archived"] = False
        ...
        with patch(
            ...
            closure = owner_mfa_vault_kms_rotation_closure_queries.get_closure(
                **self._ready_kwargs(probe_reference="owners/hashicorp-rotation-closure/blocked"),
                **closure_kwargs,
            )

        self.assertFalse(closure["ready"])
        self.assertEqual(closure["status"], "blocked")
        self.assertIn("closure:rotation-evidence-not-archived", closure["blockers"])
    ...
    def test_closure_rolls_back_when_monitoring_has_rollback_signal(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation-closure/rollback")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        kwargs = self._ready_kwargs(probe_reference="owners/hashicorp-rotation-closure/rollback")
        ...
        self.assertFalse(closure["ready"])
        self.assertEqual(closure["status"], "rollback")
        self.assertIn("monitoring:rollback-signal-present", closure["blockers"])
    ...
    def test_command_outputs_closure_without_secret_or_path(self):
        self._factor(secret_reference="ref:owners/hashicorp-rotation-closure/command")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        output = StringIO()
        ...
        with patch(
            ...
            call_command(
                "owner_mfa_vault_kms_rotation_closure",
                "--canary-tenant-id",
                str(self.canary.id),
                ...
                "--probe-reference",
                "owners/hashicorp-rotation-closure/command",
                "--canary-owner-email",
                self.owner.email,
                *self._ready_flags(),
                *self._closure_flags(),
        ...
        self.assertIn("resume_guardrail=", output.getvalue())
        self.assertNotIn(self.secret, output.getvalue())
        self.assertNotIn("owners/hashicorp-rotation-closure/command", output.getvalue())
    ...
    def _closure_kwargs(self) -> dict[str, object]:
        return {
            "rotation_closure_decision_recorded": True,
            "rotation_evidence_archived": True,
            "closure_residual_risks_accepted": True,
            "expansion_resume_plan_documented": True,
            "rollback_window_closed_or_extended": True,
            "closure_audit_evidence_ready": True,
        }
    ...
    def _closure_flags(self) -> tuple[str, ...]:
        return (
