...
class OwnerMfaHashicorpVaultProductionGateTests(TestCase):
    secret = "GEZDGNBVGY3TQOJQ"

    def setUp(self):
        self.tenant = Tenant.objects.create(name="MFA Vault Gate", slug="mfa-vault-gate", subdomain="mfa-vault-gate")
        self.owner = OwnerUser.objects.create(tenant=self.tenant, email="owner.mfa.vault.gate@hubx.market", role="owner")
    ...
    def test_gate_ready_when_readiness_and_operational_confirmations_are_ready(self):
        self._factor(secret_reference="ref:owners/hashicorp-gate/evidence")
        ...
        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            result = owner_mfa_hashicorp_vault_production_gate_queries.get_gate(
                ...
                probe_reference="owners/hashicorp-gate/evidence",
        ...
        self.assertIn("Owner MFA Vault/KMS Provider Production Activation Evidence", result["next_tracks"])
    ...
    def test_gate_blocks_missing_support_standby_confirmation(self):
        self._factor(secret_reference="ref:owners/hashicorp-gate/evidence")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        ...
        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            result = owner_mfa_hashicorp_vault_production_gate_queries.get_gate(
                ...
                rollout_order_confirmed=True,
                feature_flags_confirmed=True,
                support_standby_confirmed=False,
                rollback_window_confirmed=True,
                post_activation_monitoring_confirmed=True,
            )
        ...
        self.assertIn("confirmation-missing:support_standby_confirmed", result["blockers"])
    ...
    def test_command_outputs_gate_without_secret_or_path(self):
        self._factor(secret_reference="ref:owners/hashicorp-gate/command")
        hvac = _FakeVaultModule(payload={"data": {"data": {"totp_secret": self.secret}}})
        output = StringIO()

        with patch("app.modules.accounts.infrastructure.owner_mfa_secret_providers.importlib.import_module", return_value=hvac):
            call_command(
                "owner_mfa_hashicorp_vault_production_gate",
                "--tenant-id",
                str(self.tenant.id),
                "--probe-reference",
                "owners/hashicorp-gate/command",
                "--canary-owner-email",
                self.owner.email,
                ...
                "--post-activation-monitoring-confirmed",
                stdout=output,
            )

        self.assertIn("[READY]", output.getvalue())
        self.assertIn("decision=GO", output.getvalue())
        self.assertIn("activation_plan=", output.getvalue())
        self.assertIn("decision key=tenant-scope", output.getvalue())
        self.assertNotIn(self.secret, output.getvalue())
        self.assertNotIn("owners/hashicorp-gate/command", output.getvalue())
