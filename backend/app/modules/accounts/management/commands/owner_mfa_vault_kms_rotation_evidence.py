...
class Command(BaseCommand):
    help = "Captura evidência declarativa da rotação Vault/KMS MFA owner/admin."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"[{str(evidence['status']).upper()}] result={evidence['result']} canary_tenant_id={evidence['canary_tenant_id']} "
            f"current_target_tenant_id={evidence['current_target_tenant_id']} target_provider={evidence['target_provider']}"
        )
        ...
        if options["fail_on_blockers"] and evidence["blockers"]:
            raise CommandError("Owner MFA Vault/KMS rotation evidence is blocked.")
