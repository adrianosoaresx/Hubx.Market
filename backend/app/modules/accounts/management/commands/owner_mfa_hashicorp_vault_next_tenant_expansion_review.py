...
class Command(BaseCommand):
    help = "Revisa se a cadência Hashicorp Vault MFA pode seguir para o próximo tenant."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            ...
            f"max_parallel_tenants={review['max_parallel_tenants']}"
        )
        for target in review["next_target_tenants"]:
            blockers = ",".join(target["blockers"]) if target["blockers"] else "none"
            self.stdout.write(
                f"next_target tenant_id={target['tenant_id']} slug={target['tenant_slug']} "
                f"ready={str(target['ready']).lower()} blockers={blockers}"
            )
        for key, value in review["cadence_signals"].items():
            self.stdout.write(f"cadence_signal key={key} value={value}")
        ...
        if options["fail_on_blockers"] and review["blockers"]:
            raise CommandError("Owner MFA Hashicorp Vault next tenant expansion review is blocked.")
