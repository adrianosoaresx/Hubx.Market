...
class Command(BaseCommand):
    help = "Captura pacote de readiness evidence do provider Vault/KMS TOTP MFA owner/admin."
    ...
    def handle(self, *args, **options):
        ...
        health = result["provider_health_closure"]["provider_health"]
        self.stdout.write(
            f"[{status}] result={result['result']} tenant_id={result['tenant_id']} "
            f"target_provider={result['target_provider']} health_status={health['status']} "
            f"skeleton_status={result['skeleton_execution']['status']}"
        )
