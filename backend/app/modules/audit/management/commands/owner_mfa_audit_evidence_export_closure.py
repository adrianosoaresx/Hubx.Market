...
class Command(BaseCommand):
    help = "Fecha a trilha de export de evidência MFA owner/admin."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            ...
            f"format={closure['format']} export_count={closure['export_count']}"
        )
        for action in closure["mfa_actions"]:
        ...
        if options["fail_on_blockers"] and not closure["ready"]:
            raise CommandError("Owner MFA audit evidence export closure is blocked.")
